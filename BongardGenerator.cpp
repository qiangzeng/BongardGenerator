...
BongardGenerator::BongardGenerator(generator_params p, std::string root_path,
		id_type pid, id_type eid, int fold_id) :
		fold_id_(fold_id), root_path_(root_path), pid_(pid), eid_(eid), p_(p), gen_(
				rd_()) {
...
void BongardGenerator::output() {
	...
	if (!boost::filesystem::create_directory(dir)) {
		std::cout << "The files in the directory " << dir.string() << " will be rewritten." << std::endl;
	} else {
		std::cout << "Created directory " << dir.string() << "."<< std::endl;
	}

	std::string elem_rel = "element", circle_rel = "circle", rec_rel =
			"rectangle", tri_rel = "triangle", tri_down_rel = "triangle_down",
			tri_up_rel = "triangle_up";
	...
	for (auto &picture : pictures_) {
		picture->output(elem_file, circle_file, rect_file, tri_file,
				tri_up_file, tri_down_file, inside_file, north_file, east_file);
	}

	FILE *summary = fopen((dir/"summary").c_str(), "w");
	fprintf(summary,
			"#elements: [%d, %d]; #size: [%d, %d]; #min_insides: %d; max_pid: %llu max_eid: %llu\n",
