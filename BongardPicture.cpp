...
BongardPicture::BongardPicture(picture_params p) :
		p_(p), id_(-1) {
...
BongardElement* BongardPicture::createElement() {
	...
	if (fig == 1)
		return new BongardSquare(x, y, size);
	if (fig == 2)
		return new BongardCircle(x, y, size);
	if (fig == 3)
		return new BongardTriangle(x, y, size, 0);
	if (fig == 4)
		return new BongardTriangle(x, y, size, 1);
...
BongardCircle::BongardCircle(int x, int y, int diameter) :
	...
	setInnerBoundRect(x + radius_ - std::sqrt(2) / 2.0 * radius_,
			y + radius_ - std::sqrt(2) / 2.0 * radius_, std::sqrt(2) * radius_, std::sqrt(2) * radius_);
}
...
Rectangle::Rectangle(int x, int y, int width, int height) :
		leftBottom_(x, y), rightTop_(x + width, y + height), width_(width), height_(
				height) {
...
BongardElement::BongardElement(id_type id) :
		id_(id) {

}

void BongardElement::setId(id_type id) {
	id_ = id;
}
...
void BongardPicture::output(FILE *elem, FILE *circle, FILE*rec, FILE*tri, FILE *tri_up, FILE *tri_down, FILE *inside, FILE *north, FILE *east){
	for(auto &e:elems_){
...
void BongardCircle::output(FILE *circle, FILE*rec, FILE*tri, FILE *tri_up, FILE *tri_down) const{
	fprintf(circle, "%llu\n", getId());
}

void BongardSquare::output(FILE *circle, FILE*rec, FILE*tri, FILE *tri_up, FILE *tri_down) const{
	fprintf(rec, "%llu\n", getId());
}

void BongardTriangle::output(FILE *circle, FILE*rec, FILE*tri, FILE *tri_up, FILE *tri_down) const{
	fprintf(tri, "%llu\n", getId());
